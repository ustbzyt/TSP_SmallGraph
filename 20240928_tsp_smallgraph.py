class Graph_Advanced(Graph):
    ...
    def cache_edge_weights(self):
        """
        Precomputes and caches the weights of all edges in the graph, including both directions even for undirected graphs.
        ...
        for src in self.graph:
            for dest in self.graph:
                if src != dest:
                    # Cache the weight of the edge (src -> dest)
    ...
    def calculate_tour_distance(self, tour, edge_cache, current_best=None):
        ...
        for i in range(len(tour) - 1):
            src, dest = tour[i], tour[i+1]
            
            # Lookup the edge weight directly in the cache
            weight = edge_cache.get((src, dest), float('inf'))
